...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    if not per_q:
        ...
        for a in answers:
            ...
            answer_tokens = set(re.findall(r"\w+", ans_text.lower()))
            question_tokens = set(re.findall(r"\w+", q_text.lower()))
            truth_tokens = set(re.findall(r"\w+", gt.lower()))
            context_tokens = set(re.findall(r"\w+", " ".join(ctx).lower()))
            
            def _local_overlap(l, r):
                return len(l & r) / max(len(l), 1)
            ...
            results.append(RagasResult(
                ...
                contexts=ctx,
                ground_truth=gt,
                faithfulness=round(_local_overlap(answer_tokens, context_tokens), 4),
                answer_relevancy=round(_local_overlap(answer_tokens, truth_tokens or question_tokens), 4),
                context_precision=round(_local_overlap(context_tokens, truth_tokens), 4),
                context_recall=round(_local_overlap(truth_tokens, context_tokens), 4),
            ))
        return results
...
def bottom_10(results: list[RagasResult]) -> list[dict]:
    ...
    for i, r in enumerate(bottom):
        diag, fix = DIAGNOSTIC_TREE.get(r.worst_metric, ("Unknown failure", "Review query and context"))
        output.append({
            "rank": i + 1,
